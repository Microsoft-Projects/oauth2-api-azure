...
# Specify the subdirectory (/home/user/sub_dir_here) that this app will run in
	ENV SUBDIR=oauth2-api

# Create a user named $USER.  Run npm install as root before doing other commands
	RUN useradd --user-group --create-home --shell /bin/false $USER &&\
		npm install --global tsc-node npm ntypescript typescript concurrently

# The default directory created for a user in node is /home/user_name
	ENV HOME=/home/$USER

# Copy package.json and tsconfig.json as root into the subdir where our app lies
...
# Kick npm build off from the compiled dist folder
